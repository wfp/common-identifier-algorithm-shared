...
export function mapRequiredColumns(configAlgo: Config.AlgorithmColumns, configSource: Config.ColumnMap, configDestination: Config.ColumnMap) {
    ...
    if (typeof configAlgo !== 'object') {
        ...
        algorithmColumns = [...configAlgo.to_translate || [], ...configAlgo.static || [], ...configAlgo.reference || []]
    }
    ...
    const alwaysIncludeColumns = configDestination.columns.map(c => c.alias).filter(c => sourceAliases.has(c));
