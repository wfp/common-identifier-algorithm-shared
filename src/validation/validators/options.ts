...
export class OptionsValidator implements Validator.Base {
    ...
    constructor(opts: Validator.Options.Options) {
        ...
        if (unsupportedTypes.length > 0) {
            throw new Error(`Options validator array must contain only strings or numbers -- options are: ${JSON.stringify(opts)}`);
        }
        this.opts = opts;
    }
    ...
    validate = (value: unknown): Validator.Result => {
        if (typeof value !== "string" && typeof value !== "number") return {
            ok: false, kind: this.kind, message: `values must be either a number or a string`
        }
        if (!this.opts.value.includes(value)) return {
            ok: false, kind: this.kind, message: this.message()
        };
