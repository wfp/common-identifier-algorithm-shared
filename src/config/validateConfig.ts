...
export function validateConfig(config: Config.Options, region: string) {
  ...
  function checkColumnMapping(
    label: string,
    columnMappingObject: Config.ColumnMap,
  ) {
    return (
      isObject(label, columnMappingObject) ||
      isArrayOf(
        label + '.columns',
        (c: Config.Column) =>
          isObject('', c) ||
  ...
  function checkDestination(
    ...
    return (
      checkColumnMapping(label, destination) ||
      ...
      return isArrayOf(
        ...
        (v: Config.ColumnValidation) =>
          ...
          (isString('.value', v.value) &&
            ...
            isTrue(
              'same_value_for_all_rows',
              v.op === 'same_value_for_all_rows',
            ) &&
  ...
  function checkAlgorithm(algorithm: Config.Options['algorithm']) {
    ...
    if (typeof algorithm !== 'object') return 'Missing algorithm';

    function checkAlgorithmHash(
      algorithmHash: Config.Options['algorithm']['hash'],
    ) {
      return (
        isObject('algorithm.hash', algorithmHash) ||
        isOneOf(
          'algorithm.hash.strategy',
          ['SHA256', 'SCRYPT', 'ARGON2'],
          algorithmHash.strategy,
        )
      );
      // TODO: algorithm parameters that are dependent on the algorithm choice
    }

    function checkAlgorithmColumns(
      algorithmColumns: Config.Options['algorithm']['columns'],
    ) {
      return (
        isObject('algorithm.columns', algorithmColumns) ||
        isStringList(
          'algorithm.columns.to_translate',
          algorithmColumns.to_translate,
        ) ||
    ...
    function checkAlgorithmSalt(
      algorithmSalt: Config.Options['algorithm']['salt'],
    ) {
      // // check if there is a salt
      // if (typeof algorithmSalt !== "object")
      //     return "Missing algorithm.salt";

      const saltValidationError =
        isObject('algorithm.salt', algorithmSalt) ||
        isString(
          'algorithm.salt.validator_regex',
          algorithmSalt.validator_regex,
        ) ||
        isValidRegexp(
          'algorithm.salt.validator_regex',
          algorithmSalt.validator_regex,
    ...
    // check the salt
    return (
      checkAlgorithmColumns(algorithm.columns) ||
