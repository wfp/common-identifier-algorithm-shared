...
const isObject: ConfigValidator = (label: string, v: unknown) => typeof v !== 'object' ? `Missing ${label}` : undefined;
const isNumber: ConfigValidator = (label: string, v: unknown) => typeof v !== 'number' ? `${label} must be a number` : undefined;
const isString: ConfigValidator = (label: string, v: unknown) => typeof v !== 'string' ? `${label} must be a string` : undefined;
const isRegexp: ConfigValidator = (label: string, v: unknown) => !isRegExp(v) ? `${label} is not a valid JavaScript Regular Expression` : undefined;
const isNotEmptyString: ConfigValidator = (label: string, v: unknown) => typeof v === 'string' && v.length === 0 ? `${label} cannot be an empty string` : undefined;

const isOneOf  = (label: string, vs: unknown[], v: unknown) => !vs.includes(v) ? `${label} must be ${vs.join(" or ")}` : undefined;
const isOptional = (label: string, v: unknown, pred: ConfigValidator) => typeof v !== "undefined" ? pred(label, v) : undefined;
...
const checkColumns = (label: string, columns: Config.Column[]) => {
  return (
    isArrayOfCustomType<Config.Column>(
      label + '.columns',
      columns,
      (c) => (
        isObject('', c) ||
...
const checkSource = (source: Config.Options['source']) => {
  return (
    ...
    checkColumns("[source]", source.columns)
  );
}
...
const checkValidations = (validations: Config.Options["validations"], sourceColumns: string[]) => {
  ...
  for (let key of Object.keys(validations)) {
    ...
    let unsupported = (validations[key].find(v => !Object.values(SUPPORTED_VALIDATORS).includes(v.op as SUPPORTED_VALIDATORS)))
...
const checkAlgorithm = (algorithm: Config.Options["algorithm"], sourceColumns: string[]) => {
  ...
  for (let cg of colGroups) {
    ...
    for (let col of cg.value) {
      ...
      if (!sourceColumns.includes(col)) return `${cg.label}.${col} does not have a corresponding [source] column.`
    }
  }

  // check hash
  // TODO: Add support for other hashing functions
  exists = isObject('[algorithm].hash', algorithm.hash) || isOneOf('[algorithm].hash.strategy', ["SHA256"], algorithm.hash.strategy);
  if (exists) return exists;

  // check salt
  exists = isObject('[algorithm].salt', algorithm.salt) ||
  ...
  if (algorithm.salt.source === "FILE") {
    check = isObject('[algorithm].salt.value', algorithm.salt.value) ||
    ...
    if (Object.keys(algorithm.salt.value!).filter(v => ["win32", "darwin", "linux"].includes(v)).length === 0) {
