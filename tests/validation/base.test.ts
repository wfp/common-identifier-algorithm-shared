...
import { ValidatorBase } from '../../validation/base.js';
import { ValidationError, Validation, SUPPORTED_VALIDATORS } from '../../validation/Validation.js';

const TEST_MESSAGE = "OTHER MESSAGE";
const TEST_VALIDATOR = SUPPORTED_VALIDATORS.FIELD_NAME;

test("ValidatorBase abstract methods", () => {

    const v = new ValidatorBase(TEST_VALIDATOR, {op: "", value: ""})

    expect(() => v.defaultMessage()).toThrow()
    expect(() => v.validate("A", { row: { col_a: "A" }, sheet: { name: "", data: []}, column: "col_a"})).toThrow()
})
...
class TestValidator extends ValidatorBase {
    ...
    validate(value: any, _context?: Validation.Data) {
...
test("ValidatorBase result helpers", () => {
    const v = new TestValidator({ op: "", value: "" })
...
test("ValidatorBase::message", () => {
    // no message option
    {
        ...
        expect(v.validate("A")).toEqual(null);
        // @ts-ignore
    ...
    // have message option
    {
        const v = new TestValidator({ op: "", value: "", message: TEST_MESSAGE })
