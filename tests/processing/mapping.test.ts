...
import { mapRequiredColumns, isMappingOnlyDocument } from '@/processing/mapping';
...
describe("processing::mapping", () => {
  ...
  test('isMappingOnlyDocument empty config', () => {
  ...
  test('isMappingOnlyDocument algo columns specified only', () => {
    const testSheet: CidDocument = { name: '', data: [{ A: 1, B: 2, C: 3 }] };
  ...
  test('isMappingOnlyDocument src dest specified only', () => {
    const testSheet: CidDocument = { name: '', data: [{ A: 1, B: 2, C: 3 }] };
    const A: Config.AlgorithmColumns = {
      process: [],
      static: [],
