...
export class DateFieldDiffValidator implements Validator.Base {
    ...
    validate = (value: any, data?: Validation.Data): Validator.Result => {
        ...
        if (!otherFieldValue) return { ok: false, kind: this.kind, message: `target column '${this.opts.target}' is empty` }

        let originDate = attemptToParseDate(otherFieldValue);
        if (!originDate) return { ok: false, kind: this.kind, message: `target column '${this.opts.target}' must be a date` }
